            ...
            {adminTab === 'history' && (
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b border-slate-100 pb-3">
                  ...
                  <span className="text-xs text-slate-500 font-bold bg-slate-100 px-3 py-1 rounded-full border border-slate-200 shrink-0 self-start sm:self-auto">
                    現在: 全 {history.length} 件
                  </span>
                </div>
                ...
                {totalPages > 1 && (
                  <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                    <span className="text-xs text-slate-500 font-bold">
                      {history.length} 件中 { (historyPage - 1) * itemsPerPage + 1 } ～ { Math.min(historyPage * itemsPerPage, history.length) } 件目を表示
                    </span>
